...
export function useFiltroEstoque() {
  ...
  const filtrarPorEstoque = useCallback(
    (receitas: Recipe[]): Recipe[] => {
      if (despensaIngredientes.length === 0) return [];
      ...
      return receitas.filter((receita) => {
        try {
          ...
          return ingredientesDaReceita.every((ingRecObj: any) => {
            ...
            const qtdDespensa = Number(itemNoEstoque.qty) || 0;
            const unidDespensa = itemNoEstoque.unit || "un";

            const baseReceita = normalizarBase(qtdReceitaRaw, unidReceita);
            const baseDespensa = normalizarBase(qtdDespensa, unidDespensa);

            if (
              baseReceita.tipo === "unidade" &&
              baseDespensa.tipo === "unidade"
            ) {
              return baseDespensa.valor >= baseReceita.valor * 0.9;
            }

            if (baseDespensa.tipo === "massa_volume" && reqGramasMl_IA > 0) {
              return baseDespensa.valor >= reqGramasMl_IA * 0.9;
            }
