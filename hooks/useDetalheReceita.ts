...
export const useDetalheReceita = (): UseDetalheReceitaReturn => {
    ...
    useEffect(() => {
        ...
        async function buscarDados() {
            ...
            // Se for receita IA, não precisa buscar no banco
            if (tipo === "ia" || !receitaId || isNaN(Number(receitaId))) {
                if (isMounted) {
    ...
    const receitaDetalhada: ReceitaDetalhada = useMemo(() => {
        if (receitaBancoDados) {
            ...
            return {
                titulo: receitaBancoDados.nome_receita || RECEITA_STRINGS.RECEITA_DESCONHECIDA,
                descricao: receitaBancoDados.descricao_simples_preparo || RECEITA_STRINGS.DESCRICAO_INDISPONIVEL,
                tempo: formatarTempo(receitaBancoDados.tempo_preparo || `${RECEITA_STRINGS.VALOR_PADRAO} min`),
                dificuldade: receitaBancoDados.dificuldade || RECEITA_STRINGS.VALOR_PADRAO,
                calorias: receitaBancoDados.calorias || `${RECEITA_STRINGS.VALOR_PADRAO} kcal`,
    ...
    }, [isIA, receitaId, receitaDetalhada, params.ingredients, params.steps]);
