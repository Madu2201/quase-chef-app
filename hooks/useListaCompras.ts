...
export function useListaCompras() {
    ...
    const addItem = async (nome: string, qtd: string, unidade: string) => {
        ...
        if (itemExistente) {
            ...
            Alert.alert("Sucesso", `Quantidade de ${nome} atualizada para ${novaQuantidade} ${unidade}`);
            return;
        }
    ...
    setItems(prev => prev.map(i => i.id === id ? { ...i, quantidade_comprar: novaQuantidade } : i));
    ...
    const gerarListaDaDispensa = async () => {
        ...
        try {
            ...
            await buscarLista();
            Alert.alert("Pronto!", "Sua lista foi atualizada com os itens faltantes.");
        } catch (error) {
    ...
    /**
     * NOVA FUNÇÃO (FASE 2): Guarda os itens comprados na dispensa e limpa a lista
     */
    const guardarNoEstoque = async (onUpsert: (nome: string, qtd: number, unit: string) => Promise<boolean>) => {
        if (!user?.id) return;
        ...
        for (const item of itensParaGuardar) {
            const sucesso = await onUpsert(item.nome, Number(item.quantidade_comprar), item.unidade);
            if (sucesso) {
                sucessoCount++;
            }
        }
        ...
        if (sucessoCount > 0) {
            ...
            Alert.alert("Sucesso!", `${sucessoCount} itens guardados na sua dispensa.`);
        } else {
            Alert.alert("Ops", "Não foi possível guardar os itens. Verifique se as unidades são compatíveis.");
        }
    };
